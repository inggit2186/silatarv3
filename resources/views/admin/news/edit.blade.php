<x-admin.layouts.app title="{{ $title }}">
    <div class="min-h-screen">
        ...
        <div class="mb-8">
            ...
            <p class="mt-2 text-sm text-slate-400">Perbarui berita atau pengumuman untuk portal SILATAR</p>
        </div>
            ...
            <div class="grid gap-6 lg:grid-cols-3">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="rounded-xl border border-cyan-500/20 bg-slate-900/60 p-6">
                        ...
                        <input type="text" name="title" value="{{ old('title', $news->title) }}" required class="w-full rounded-lg border border-cyan-500/30 bg-slate-900/80 px-4 py-3 font-mono text-white placeholder-slate-500 focus:border-cyan-400 focus:outline-none focus:shadow-[0_0_15px_rgba(0,212,255,0.3)]" placeholder="Masukkan judul berita...">
                        @error('title')
                    ...
                    <div class="rounded-xl border border-cyan-500/20 bg-slate-900/60 p-6">
                        ...
                        <!-- Editor -->
                        <div id="editor" contenteditable="true" class="min-h-[300px] w-full rounded-lg border border-cyan-500/30 bg-slate-900/80 p-4 text-white focus:border-cyan-400 focus:outline-none focus:shadow-[0_0_15px_rgba(0,212,255,0.3)]" oninput="updateHiddenInput()">{!! old('content', $news->content) !!}</div>
                        <input type="hidden" name="content" id="contentInput" value="{{ old('content', $news->content) }}">
                    ...
                    <div class="rounded-xl border border-violet-500/20 bg-slate-900/60 p-6">
                        ...
                        <div class="mb-4">
                            ...
                            <div class="relative">
                                ...
                                <span id="meta_desc_count" class="absolute right-3 bottom-3 text-xs text-slate-500">{{ strlen(old('meta_description', $news->meta_description ?? '')) }}/160</span>
                            </div>
                        ...
                        <div class="mt-4 p-4 rounded-lg border border-slate-700/50 bg-slate-900/40">
                            ...
                            <div id="seo_preview" class="font-sans">
                                <p id="seo_preview_title" class="text-lg text-[#b4d7ff] hover:underline cursor-pointer mb-0.5">{{ $news->meta_title ?: $news->title }} - SILATAR</p>
                                <p id="seo_preview_url" class="text-sm text-[#bdc3c7] mb-0.5 truncate">tanahdatar.kemenag.go.id/berita/{{ $news->slug }}</p>
                                <p id="seo_preview_desc" class="text-sm text-[#9aa5b1]">{{ $news->meta_description ?: Str::limit($news->excerpt, 160) }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="rounded-xl border border-cyan-500/20 bg-slate-900/60 p-6">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="datetime-local" name="publish_date" value="{{ old('publish_date', $news->publish_date ? \Carbon\Carbon::parse($news->publish_date)->format('Y-m-d\TH:i') : \Carbon\Carbon::now()->format('Y-m-d\TH:i')) }}" class="w-full rounded-lg border border-cyan-500/30 bg-slate-900/80 py-2.5 px-3 font-mono text-sm text-white focus:border-cyan-400 focus:outline-none">
                            </div>

                            <div class="flex items-center gap-3">
                                <input type="checkbox" name="is_featured" id="is_featured" value="1" {{ old('is_featured', $news->is_featured) ? 'checked' : '' }} class="h-4 w-4 rounded border-cyan-500/30 bg-slate-900/80 text-cyan-500 focus:ring-cyan-500 focus:ring-offset-slate-900">
                                <label for="is_featured" class="text-sm font-mono text-slate-300">Tampilkan di Featured</label>
                            </div>

                            <div class="flex items-center gap-3">
                                <input type="checkbox" name="is_slideshow" id="is_slideshow" value="1" {{ old('is_slideshow', $news->is_slideshow) ? 'checked' : '' }} class="h-4 w-4 rounded border-cyan-500/30 bg-slate-900/80 text-cyan-500 focus:ring-cyan-500 focus:ring-offset-slate-900">
                    ...
                    <div class="rounded-xl border border-cyan-500/20 bg-slate-900/60 p-6">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" name="writer" value="{{ old('writer', $news->writer ?? '') }}" class="w-full rounded-lg border border-cyan-500/30 bg-slate-900/80 py-2.5 px-3 font-mono text-sm text-white placeholder-slate-500 focus:border-cyan-400 focus:outline-none" placeholder="Nama penulis (contoh: Budi, Siti)...">
                            </div>

                            <div>
                                <label class="mb-2 block text-xs font-mono uppercase tracking-wider text-slate-400">Editor</label>
                                <input type="text" name="editor" value="{{ old('editor', $news->editor ?? '') }}" class="w-full rounded-lg border border-cyan-500/30 bg-slate-900/80 py-2.5 px-3 font-mono text-sm text-white placeholder-slate-500 focus:border-cyan-400 focus:outline-none" placeholder="Nama editor (opsional)...">
                            </div>

                            <div>
                                <label class="mb-2 block text-xs font-mono uppercase tracking-wider text-slate-400">Fotografer</label>
                                <input type="text" name="photographer" value="{{ old('photographer', $news->photographer ?? '') }}" class="w-full rounded-lg border border-cyan-500/30 bg-slate-900/80 py-2.5 px-3 font-mono text-sm text-white placeholder-slate-500 focus:border-cyan-400 focus:outline-none" placeholder="Nama fotografer (opsional)...">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="rounded-xl border border-cyan-500/20 bg-slate-900/60 p-6">
                        ...
                        @if($news->image)
                        <div class="mb-4 rounded-lg overflow-hidden border border-cyan-500/20">
                        ...
                        <div class="relative">
                            ...
                            <label for="imageInput" class="flex min-h-[160px] cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-cyan-500/30 bg-slate-900/40 transition-all hover:border-cyan-400/50 hover:bg-cyan-500/5 overflow-hidden">
                                <div id="imagePreview" class="{{ $news->image ? 'hidden' : '' }} w-full h-full absolute inset-0">
                                    <img id="previewImg" class="w-full h-full object-cover">
                                </div>
                                <div id="imagePlaceholder" class="{{ $news->image ? 'hidden' : '' }} flex flex-col items-center justify-center text-center p-4">
                                    <svg class="h-8 w-8 text-slate-500 mb-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/>
                                    </svg>
                                    <p class="text-xs text-slate-400">Klik untuk change gambar</p>
                                </div>
                            </label>
                        </div>
                        @error('image')
    ...
    <script>
        ...
        document.addEventListener('click', function(e) {
            ...
            if (sizeBtn) {
                console.log('Size button clicked:', sizeBtn.dataset.size);
                setImageSize(sizeBtn.dataset.size);
            }
            if (alignBtn) {
                console.log('Align button clicked:', alignBtn.dataset.align);
        ...
        console.log('Align buttons found:', document.querySelectorAll('.align-btn').length);

        function insertContentImage() {
            const urlInput = document.getElementById('imageUrlInput').value;
            const caption = document.getElementById('imageCaption').value;

            // Get image URL (either uploaded or URL input)
            if (pendingImageFile) {
        ...
        function updateSEOPreview() {
            ...
            if (previewTitle) previewTitle.textContent = (metaTitle.length > 60 ? metaTitle.substring(0, 57) + '...' : metaTitle) + ' - SILATAR';
