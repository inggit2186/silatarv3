{{-- Partial: Kriteria Item untuk Form Edit (NEO MIRAI Theme) --}}
...
<div class="kriteria-card" x-data="{
        ...
        <div class="kriteria-icon {{ $iconColor }}">
            @switch($kriteriaInfo['icon'])
                @case('heart-handshake')
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z"/><path d="m12 13-1-1 2-2-3-3 2-2"/><path d="m18 13 1-1-2-2 3-3-2-2"/></svg>
                @break
                @case('shield-check')
    ...
    <div class="mb-4">
        ...
        >{{ old("kriteria.{$kriteriaKey}.catatan", $data['catatan']) }}</textarea>
    </div>
    ...
    <div class="thumbs-controls">
        <div class="thumbs-group">
            ...
            <span class="thumbs-count positive" x-text="up">{{ $data['thumbs_up'] }}</span>
        ...
        <div class="thumbs-group">
            <span class="thumbs-label">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M10 15v4a3 3 0 0 0 3 3l4-9V2H5.72a2 2 0 0 0-2 1.7l-1.38 9a2 2 0 0 0 2 2.3zM17 2h2.67A2.31 2.31 0 0 1 22 4v7a2.31 2.31 0 0 1-2.33 2H17"/></svg>
                Kurang
            </span>
            <button type="button" @click="down = Math.max(0, down - 1)" class="thumbs-btn minus">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="5" y1="12" x2="19" y2="12"/></svg>
            </button>
            <span class="thumbs-count negative" x-text="down">{{ $data['thumbs_down'] }}</span>
