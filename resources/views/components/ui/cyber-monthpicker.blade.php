...
<div x-data="silatarMonthpicker(@js([
    ...
    <div class="relative">
        ...
        >
            ...
            {{-- Text --}}
            <span class="block pl-12 pr-20 font-mono text-sm" :class="value ? 'text-cyan-300' : 'text-slate-500'" x-text="displayText()"></span>
            ...
            >
                ...
                <div class="relative p-5">
                    ...
                    <div class="mb-4 flex items-center justify-between gap-6">
                        <button type="button" @click="prevYear()" class="group flex h-10 w-10 items-center justify-center rounded-xl border border-cyan-500/30 bg-slate-800/50 text-cyan-400 transition-all hover:border-cyan-400/50 hover:bg-cyan-500/20 hover:shadow-[0_0_15px_rgba(0,212,255,0.3)]">
                        ...
                        <input
                            type="number"
                            class="w-28 rounded-xl border border-cyan-500/30 bg-slate-800/80 px-4 py-2.5 text-center font-mono text-lg font-bold text-cyan-300 shadow-[inset_0_2px_4px_rgba(0,0,0,0.3)] transition focus:border-cyan-400 focus:shadow-[0_0_20px_rgba(0,212,255,0.2),inset_0_2px_4px_rgba(0,0,0,0.3)] focus:outline-none"
                            :value="yearCursor"
                        ...
                        <button type="button" @click="nextYear()" class="group flex h-10 w-10 items-center justify-center rounded-xl border border-cyan-500/30 bg-slate-800/50 text-cyan-400 transition-all hover:border-cyan-400/50 hover:bg-cyan-500/20 hover:shadow-[0_0_15px_rgba(0,212,255,0.3)]">
                            <svg class="h-5 w-5 transition-transform group-hover:translate-x-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    ...
                    <div class="grid grid-cols-3 gap-2">
                        <template x-for="month in monthItems" :key="month.key">
                            <button
                                type="button"
                                class="relative rounded-xl border px-3 py-3 font-mono text-xs font-semibold transition-all"
                                :class="isSelected(month.index)
                            ...
                            >
                                ...
                                {{-- Selected indicator --}}
                                <span x-show="isSelected(month.index)" class="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-cyan-400 shadow-[0_0_10px_rgba(0,212,255,0.5)]">
                                    <svg class="h-2.5 w-2.5 text-slate-950" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
                    ...
                    {{-- Footer --}}
                    <div class="mt-4 flex items-center justify-between gap-3 border-t border-cyan-500/20 pt-4">
                        <button type="button" @click="clearValue()" class="flex items-center gap-2 rounded-lg border border-rose-500/30 bg-rose-500/10 px-4 py-2 font-mono text-xs font-semibold text-rose-400 transition-all hover:border-rose-400/50 hover:bg-rose-500/20">
                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                            </svg>
                            Hapus
                        </button>
                        <button type="button" @click="closePicker()" class="flex items-center gap-2 rounded-lg bg-gradient-to-r from-cyan-600 to-cyan-500 px-5 py-2 font-mono text-xs font-bold uppercase tracking-wider text-white shadow-[0_0_20px_rgba(0,212,255,0.3)] transition-all hover:shadow-[0_0_30px_rgba(0,212,255,0.5)]">
                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/>
                            </svg>
                            Pilih
